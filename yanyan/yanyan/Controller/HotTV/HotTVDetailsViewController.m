...
@interface HotTVDetailsViewController ()

@property(nonatomic,strong)HotCommentM * m_commentModel;
@end

@implementation HotTVDetailsViewController
...
{
     return [self.m_tableView cellHeightForIndexPath:indexPath model:self.m_commentModel keyPath:@"m_hotCommentM" cellClass:[HotTVOneCommentsDetaTableViewCell class] contentViewWidth:[UIScreen mainScreen].bounds.size.width];
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    NSString * cellid = @"HotTVOneCommentsDetaTableViewCell";
    
    HotTVOneCommentsDetaTableViewCell *cell = [tableView cellForRowAtIndexPath:indexPath]; //根据indexPath准确地取出一行，而不是从cell重用队列中取出 //
    if (cell == nil) {
        
        // 是  TableViewCell 不是之前的系统的 UITableViewCell
        cell = [[HotTVOneCommentsDetaTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellid];
    }
