...
- (void)setColumnNumber:(NSInteger)columnNumber {
    ...
    CGFloat margin = 3;
    CGFloat itemWH = (TZScreenWidth - 2 * margin - 3) / columnNumber - margin;
    ...
    if (iOS8Later) {
        ...
        for (PHFetchResult *fetchResult in allAlbums) {
            for (PHAssetCollection *collection in fetchResult) {
                ...
                if ([collection.localizedTitle containsString:@"Deleted"] || [collection.localizedTitle isEqualToString:@"最近删除"] || [collection.localizedTitle isEqualToString:@"最近添加"]||[collection.localizedTitle isEqualToString:@"最近删除"]||[collection.localizedTitle isEqualToString:@"视频"]||[collection.localizedTitle isEqualToString:@"慢动作"] ||[collection.localizedTitle isEqualToString:@"已隐藏"]||[collection.localizedTitle isEqualToString:@"My Photo Stream"] || [collection.localizedTitle isEqualToString:@"我的照片流"])
                    continue;
                if ([self isCameraRollAlbum:collection.localizedTitle]) {
...
//针对大体积图的获取方法  自己写的
- (PHImageRequestID)getPhotoWithAssetXSD:(id)asset completion:(void (^)(UIImage *, NSDictionary *, BOOL isDegraded))completion {
    CGFloat fullScreenWidth = TZScreenWidth;
    if (fullScreenWidth > _photoPreviewMaxWidth) {
        fullScreenWidth = _photoPreviewMaxWidth;
    }
    return [self getPhotoWithAssetXSD:asset photoWidth:fullScreenWidth completion:completion progressHandler:nil networkAccessAllowed:YES];
}
//同上
- (PHImageRequestID)getPhotoWithAssetXSD:(id)asset photoWidth:(CGFloat)photoWidth completion:(void (^)(UIImage *photo,NSDictionary *info,BOOL isDegraded))completion progressHandler:(void (^)(double progress, NSError *error, BOOL *stop, NSDictionary *info))progressHandler networkAccessAllowed:(BOOL)networkAccessAllowed {
    if ([asset isKindOfClass:[PHAsset class]]) {
        CGSize imageSize;
        if (photoWidth < TZScreenWidth && photoWidth < _photoPreviewMaxWidth) {
            imageSize = AssetGridThumbnailSize;
            ...
            CGFloat ratio=  [DTools getRatioSize: phAsset.pixelWidth  bitHeight: phAsset.pixelHeight ];
            
            imageSize = CGSizeMake(phAsset.pixelWidth*ratio, phAsset.pixelHeight*ratio);
